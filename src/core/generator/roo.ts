...
export class RooGenerator extends BaseGenerator {
  readonly toolName = "roo";
  readonly outputPath = ".roo/rules.md";
  readonly description = "Roo Code project rules";

  generate(profile: ProjectProfile, config: AirulesConfig): string {
    const lines: string[] = [];
    lines.push(`# Roo Code Rules for ${config.project.name}`);
